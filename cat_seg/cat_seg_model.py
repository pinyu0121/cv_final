        ...
        *,
        ...
        for name, params in self.sem_seg_head.predictor.clip_model.named_parameters():
            if "transformer" in name:
                ...
                elif clip_finetune == "oft":
                    #set_oft(self.sem_seg_head.predictor.clip_model.transformer, ResidualAttentionBlock, dim=8, hidden_size=512, r=)
                    if "visual" in name:
                ...
                elif clip_finetune == "svd":
                    #print(name)
                    if "visual" in name:
                        if "attn" in name or "position" in name:
                            params.requires_grad = True
                    elif "vector_S" in name:
                        # QV fine-tuning for attention blocks
                        print(name)
                        params.requires_grad = True #if "q_proj" in name or "v_proj" in name else False
                    elif "position" in name:
